document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (pageTitle) {
    pageTitle.textContent = pageTitles[currentPath] || 'Stream Team HQ';
  }
  ...
  async function loadNotifications() {
    try {
      ...
      if (!notifs || notifs.length === 0) {
        list.innerHTML = '<p style="color: #94a3b8; padding: 20px; text-align: center;">Aucune notification</p>';
        return;
      }
      ...
      notifs.forEach(n => {
        ...
        item.innerHTML = `
          ...
          <div class="notif-content">
            ...
            ${n.message}
            <small>${new Date(n.created_at).toLocaleString()} • ${n.category?.charAt(0).toUpperCase() + n.category?.slice(1)}</small>
          </div>
        `;
  ...
  function renderCards(streams, users) {
    ...
    const sortedUsers = [...users].sort((a, b) => 
      liveUsers.includes(b.login) - liveUsers.includes(a.login)
    );
    ...
    sortedUsers.forEach(user => {
      ...
      card.innerHTML = `
        ...
        ${isLive ? `<p class="details">🎮 ${stream.game_name}</p>` : '<p class="details offline-text">📴 hors-ligne</p>'}
        ${isLive ? `<p class="details">🎧 ${stream.title}</p>` : ''}
        ${isLive ? `<p class="details">👥 ${stream.viewer_count} spectateurs</p>` : ''}
        ${isLive ? `<a href="https://twitch.tv/${user.login}" target="_blank">Regarder en direct</a>` : ''}
