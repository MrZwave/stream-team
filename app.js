...
app.get('/auth/twitch', (req, res) => {
  ...
  res.redirect(`https://id.twitch.tv/oauth2/authorize?client_id=${process.env.TWITCH_CLIENT_ID}&redirect_uri=${redirect_uri}&response_type=code&scope=user:read:email user:edit:follows`);
});
...
app.get('/api/clips', async (req, res) => {
  ...
  try {
    ...
    const twitchRes = await fetch(`https://api.twitch.tv/helix/clips?broadcaster_id=${broadcasterId}`, {
      headers: {
        'Client-ID': process.env.TWITCH_CLIENT_ID,
...
app.get('/api/followers', async (req, res) => {
  ...
  try {
    ...
    const twitchRes = await fetch(`https://api.twitch.tv/helix/users/follows?to_id=${userId}`, {
      headers: {
        'Client-ID': process.env.TWITCH_CLIENT_ID,
        'Authorization': `Bearer ${accessToken}`
      }
...
app.get('/api/profile-stats', async (req, res) => {
  ...
  try {
    ...
    await db.query('UPDATE streamers SET clicks = clicks + 1 WHERE login = ?', [login]);
...
app.post('/api/salve', async (req, res) => {
  ...
  try {
    ...
    const [[{ salves }]] = await db.query(`SELECT salves FROM streamers WHERE login = ?`, [login]);
...
const checkLiveStreams = async () => {
  try {
    ...
    const twitchRes = await fetch(`https://api.twitch.tv/helix/streams?${query}`, {
      headers: {
        'Client-ID': process.env.TWITCH_CLIENT_ID,
...
app.get('/api/live', async (req, res) => {
  try {
    ...
    const token = await getAppAccessToken();
    const qs = users.map(u => `user_login=${encodeURIComponent(u)}`).join('&');
...
app.get('/api/admin/streamers', requireAdmin, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT id, login, display_name, is_admin FROM streamers');
    res.json(rows);
  } catch (err) {
...
app.get('/api/admin/user-cards/:id', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (err.code === 'ER_NO_SUCH_TABLE') {
      console.warn('🔧 Table user_cards ou card_templates manquante. Retourne []');
...
app.post('/api/admin/quests', requireAdmin, async (req, res) => {
  const { name, description, type, xp_reward, requirement, card_reward } = req.body;
  ...
  try {
    ...
    res.status(500).json({ error: 'Erreur serveur lors de l\'ajout de la quête.' });
  }
});
...
app.get('/streamer/:name', async (req, res) => {
  ...
  try {
    ...
    const clipsRes = await fetch(`https://api.twitch.tv/helix/clips?broadcaster_id=${userData.id}&first=6`, {
      headers: {
        'Client-ID': process.env.TWITCH_CLIENT_ID,
        'Authorization': `Bearer ${token}`
      }
    ...
    const [rows] = await db.query('SELECT clicks, salves FROM streamers WHERE login = ?', [name]);
